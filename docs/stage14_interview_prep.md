...
This project solves the analytical gap that arises when e-commerce operations data lives in isolation from the external context that explains its behaviour. By consolidating three heterogeneous feeds — Olist Brazilian e-commerce transactions (99k orders), Open-Meteo historical weather (daily observations for 20 cities), and Frankfurter FX rates (daily USD/BRL) — into a single unified warehouse, the pipeline makes cross-domain questions answerable that were previously blocked by schema and granularity mismatches. The architecture follows the medallion pattern: Bronze preserves raw Parquet snapshots and source tables; Silver enforces pandera schemas and routes rejected rows to a quarantine layer with explicit rejection reasons; Gold materializes a star schema (5 dimensions + 4 facts in Gold Parquet: `fact_sales`, `fact_weather_daily`, `fact_fx_rates`, `fact_payments`; the first three are loaded into PostgreSQL today, `fact_payments` is Parquet-only until its warehouse loader entry lands). The output is a production-grade analytics foundation paired with a 4-page Power BI semantic-model design (27 DAX measures specified in `docs/stage9_dax_measures.md`; the `.pbix` is authored separately on a Windows VM per `docs/powerbi_vm_workflow.md`), enabling revenue analysis by geography, weather-to-delivery correlation (via `analytics.v_sales_with_weather`), and multi-currency normalization (via `analytics.v_sales_usd`).
...
Data flows through three entry points — the Olist dataset (9 CSVs, ~99k orders), Open-Meteo API (daily weather for top 20 cities by order volume), and Frankfurter API (daily FX rates) — into the Bronze layer, where raw Parquet snapshots are persisted alongside a PostgreSQL `source_system` schema replicating each source's table structure. The Silver layer applies pandera schemas in `lazy=True` mode, collecting all violations before raising so a single row can be tagged with multiple rejection reasons; rejected rows go to a `quarantine/` directory with `quarantine_reason` and `quarantined_at` columns. The Gold layer materializes a star schema across four facts (`fact_sales` at order line-item grain, `fact_weather_daily` at city+date, `fact_fx_rates` at date+currency pair, `fact_payments` at order+payment_sequential grain) and five dimensions. The first three facts plus all dimensions are written to both Parquet and PostgreSQL's `analytics` schema; `fact_payments` is Gold-Parquet-only today (warehouse loader entry pending). Orchestration runs in two modes: `--full-refresh` runs extract→silver→gold→warehouse→quality; `--incremental` skips extract and reuses cached Bronze for faster re-runs when APIs are unavailable. The warehouse is the consumption surface for a Power BI Import-mode model (4-page layout + 27 DAX measures designed in `docs/stage8–10*`); the `.pbix` is authored on a Windows VM (`docs/powerbi_vm_workflow.md`).

---

### Q2: How did you handle API failures?

Coverage is uneven by source. The Open-Meteo weather extractor (`src/extract/extract_weather.py`) wraps each city request in a hand-rolled bounded-retry loop with exponential backoff, allowing transient HTTP failures to retry before eventually logging and degrading gracefully. The Frankfurter FX extractor (`src/extract/extract_fx.py`) does not retry — it relies on the API being a stable public endpoint and on Bronze caching. The Bronze layer persists API responses on disk (JSON for weather and FX, Parquet for DB snapshots), so `--incremental` simply skips extraction and reuses cached data — the pipeline does not fail if an API is temporarily down. Extraction errors are logged via loguru with full context (timestamp, source, attempt count); the Silver-layer pandera validation then verifies that stale cached data still conforms to schema. If validation rejects rows, they go to the quarantine layer with explicit `quarantine_reason` labels, and post-load quality checks in `analytics.data_quality_log` flag elevated rejection rates so the problem surfaces before the dashboard consumes bad data. Honest gap to call out in an interview: FX is one external dependency away from a brittle path, and adding a tenacity-decorated retry there is the smallest hardening change available.
